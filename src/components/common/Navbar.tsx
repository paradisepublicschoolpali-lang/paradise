...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      ...
      <div className="bg-slate-900 text-white text-xs py-2 px-3 sm:px-4 border-b border-slate-800">
          ...
          <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 lg:hidden shrink-0">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-blue-50 hover:bg-blue-100 active:bg-blue-200 border border-blue-200 text-slate-800 transition-colors cursor-pointer flex items-center justify-center"
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-2 shadow-lg">
            <div className="grid grid-cols-2 gap-2 pt-2">
              {navLinks.map(link => (
                <button
                  key={link.id}
                  onClick={() => navigateTo(link.id)}
                  className={`p-3 text-left rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors cursor-pointer ${
                    activeTab === link.id
            ...
            <div className="pt-4 flex flex-col gap-2 border-t border-slate-100">
              ...
              <button
                onClick={() => navigateTo('admissions')}
                className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer shadow-sm"
              >
